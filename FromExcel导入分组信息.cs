...
// 从指定的Excel文件中读取数据，并根据“短标题”将文献自动分配到对应的分组。
...
// 使用说明：本项目是从excel中导入数据，如果excel的“Shorttitle”（47行）的名字在库中的shorttitle有，则将对应的"Group"列导入（48行）
public static class CitaviMacro
{
	public static void Main()
	{
		...
		bool createMissingReference = false; // 不创建新条目！！
		ReferencePropertyId targetField = ReferencePropertyId.Categories;
		...
		string columnNameShortTitle = "Shorttitle";		//required - needed to identify a reference in the active project
		string columnNameLabel = "Group";				//required - field content to import
		...
		for (int i = 1; i < dataTable.Rows.Count; i++) 
		{
			...
			else 
			{
				...
				if (parentReference != null && !string.IsNullOrEmpty(label))
				{
					...
					//这里是针对分组之类的
					DebugMacro.WriteLine(label);
					Group mygroup = activeProject.Groups.FirstOrDefault(item => item.Name.Equals(label, StringComparison.OrdinalIgnoreCase));
					if (mygroup == null)
					{
						mygroup = activeProject.Groups.Add(label);
					}
					parentReference.Groups.Add(mygroup);
