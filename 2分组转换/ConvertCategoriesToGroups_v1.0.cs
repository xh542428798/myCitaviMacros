	...
	public static void Main()
	{
		...
		*/
		...
		Dictionary<string, Group> groupDictionary = new Dictionary<string, Group>();
			foreach(Category keyword in activeProject.Categories)
			{
				Group group = activeProject.Groups.FirstOrDefault(item => item.Name.Equals(keyword.FullName, StringComparison.OrdinalIgnoreCase));
				if (group == null)
				{
					group = activeProject.Groups.Add(keyword.FullName);
				}
				
				groupDictionary[keyword.FullName] = group;
			}
			...
			foreach (Reference reference in activeProject.References)
			{
				...
				{
					reference.Groups.Add(groupDictionary[referenceCategory.FullName]);
				}
			}
