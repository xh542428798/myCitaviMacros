...
public static class CitaviMacro
{
    public static void Main() 
    {
		...
		// --- 【新增】调用修改颜色的方法 ---
        ChangeAnnotationColor(knowledge);
    }

/// <summary>
/// 【修正版】修改指定知识条目所关联的注释的颜色
/// </summary>
/// <param name="knowledgeItem">要修改的知识条目</param>
public static void ChangeAnnotationColor(KnowledgeItem knowledgeItem)
{
    if (knowledgeItem == null) return;

    // 1. 从知识条目的 EntityLinks 中找到指向 Annotation 的链接
    var annotationLink = knowledgeItem.EntityLinks
        .FirstOrDefault(el => el.Indication == EntityLink.PdfKnowledgeItemIndication && el.Target is SwissAcademic.Citavi.Annotation);

    if (annotationLink == null)
    {
        MessageBox.Show("该知识条目没有关联到任何PDF注释。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
        return;
    }

    // 2. 获取 Citavi 的 Annotation 对象
    SwissAcademic.Citavi.Annotation citaviAnnotation = annotationLink.Target as SwissAcademic.Citavi.Annotation;
    if (citaviAnnotation == null) return;

    // 3. 【关键】通过反射获取底层的 PDFTron Annot 对象
    // Citavi的Annotation内部有一个字段叫 "_annot"，它指向真正的PDFTron注释
    FieldInfo pdftronAnnotField = typeof(SwissAcademic.Citavi.Annotation).GetField("_annot", BindingFlags.Instance | BindingFlags.NonPublic);
    if (pdftronAnnotField == null)
    {
        MessageBox.Show("无法访问底层PDF注释对象。Citavi版本可能已更改。", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
        return;
    }

    object pdftronAnnotObject = pdftronAnnotField.GetValue(citaviAnnotation);
    if (pdftronAnnotObject == null) return;

    // 4. 定义你想要的新颜色，例如，亮绿色
    System.Drawing.Color newColor = System.Drawing.Color.LightGreen;

    // 5. 【核心】调用PDFTron对象的方法来设置颜色
    // 我们需要将 System.Drawing.Color 转换为PDFTron的 ColorPt
    Type pdftronAnnotType = pdftronAnnotObject.GetType();
    MethodInfo setColorMethod = pdftronAnnotType.GetMethod("SetColor", new Type[] { typeof(pdftron.PDF.ColorPt) });
    
    if (setColorMethod != null)
    {
        // 创建PDFTron的颜色对象 (R, G, B 分量范围是 0.0 到 1.0)
        var colorPt = new pdftron.PDF.ColorPt(newColor.R / 255.0, newColor.G / 255.0, newColor.B / 255.0);
        setColorMethod.Invoke(pdftronAnnotObject, new object[] { colorPt });
    }
    else
    {
        MessageBox.Show("无法找到设置颜色的方法。Citavi版本可能已更改。", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
        return;
    }

    // 6. 【重要】强制刷新PDF注释的外观
    // 仅仅设置颜色是不够的，还需要让它重绘
    MethodInfo refreshAppearanceMethod = pdftronAnnotType.GetMethod("RefreshAppearance");
    if (refreshAppearanceMethod != null)
    {
        refreshAppearanceMethod.Invoke(pdftronAnnotObject, null);
    }

    // 7. 【重要】刷新Citavi的预览窗口
    Location location = citaviAnnotation.Location;
    if (location != null)
    {
    ...
    MessageBox.Show("注释颜色已修改！", "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
}
