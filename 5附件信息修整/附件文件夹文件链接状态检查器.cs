...
// 功能：
// 这个宏更侧重于文件管理。它会扫描一个指定文件夹（默认是Citavi项目的附件文件夹），
// 然后分析哪些文件已经被项目链接了，哪些还没有，并生成详细的报告。

// 具体逻辑：
// 确定要检查的文件夹路径（默认是AttachmentsFolderPath）。
// 收集项目中所有链接到参考文献和知识项的文件路径。
// 将文件夹中的所有文件与项目已链接的文件列表进行比对。
// 生成5个文本文件，分别记录：
// 项目中所有链接的文件
// 项目中所有失效的链接（文件已不存在）
// 被检查文件夹中的所有文件
