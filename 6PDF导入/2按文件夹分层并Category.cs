 ...
 * -------------------------------------------------------------------------------------
 * 最终效果：
 * 在Citavi项目中，你会得到一个清晰的层级结构：
 * - 父条目 (BookEdited): "格氏解剖学" (类别: 格氏解剖学)
 *   ├─ 子条目 (Contribution): "01_绪论" (类别: 格氏解剖学, 附件: 链接)
 *   └─ 子条目 (Contribution): "02_上肢" (类别: 格氏解剖学, 附件: 链接)
 *
 * =====================================================================================
 */
public static class CitaviMacro
{
	public static void Main()
	{
		...
		{	
			...
			foreach (string subDirPath in subDirectories)
			{
				...
				// 【补回功能】为父条目添加类别（这里类别就是它的文件夹名）
				AddCategories(new Reference[] { parentReference }, parentFolderName);
			...
			foreach (var parentPair in parentReferences)
			{
				...
				foreach (FileInfo pdfFile in pdfFiles)
				{
					...
					string directoryPath = Path.GetDirectoryName(filePath); // 获取文件所在目录的完整路径
					string parentFolderName = Path.GetFileName(directoryPath); // 获取直接父文件夹的名称
					string thisfileName = Path.GetFileNameWithoutExtension(filePath);
					string fileName = string.Format("{0}_{1}", parentFolderName, thisfileName); // 组合成 "文件夹名_文件名"
					...
					// 【补回功能】为子条目也添加和父条目相同的类别
					AddCategories(new Reference[] { childReference }, Path.GetFileName(parentDirPath));
					
					childCounter++;
				}
			}
			
			// 【步骤2】处理根目录下的所有独立PDF（创建单本书）
			...
			foreach (FileInfo pdfFile in rootPdfFiles)
			{
				...
				newReference.ReferenceType = ReferenceType.Book; // 【关键】设置为Book
	...
	/// <summary>
	/// 根据给定的类别字符串（如 "01.1格氏解剖学"）为参考文献创建并分配类别。
	/// 注意：此版本简化了，因为类别字符串不再包含路径分隔符。
	/// </summary>
	private static void AddCategories(IEnumerable<Reference> references, string categoryString)
	{
		if (string.IsNullOrEmpty(categoryString) || references == null || !references.Any())
		{
			return;
		}
		...
		// 查找或创建类别
		var currentCategory = project.Categories.FirstOrDefault(item => item.Name.Equals(categoryString, StringComparison.Ordinal));
		if (currentCategory == null)
		{
			currentCategory = project.Categories.Add(categoryString);
		}
